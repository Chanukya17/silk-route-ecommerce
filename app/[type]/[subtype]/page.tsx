...
export default async function SubtypePage({
  ...
  // Deslugify subtype (e.g. kanjivaram -> Kanjivaram)
  // Our database currently uses exact case-sensitive matches for Subtype name if not handled carefully, 
  // but we can use mode: 'insensitive'
  const subtypeSlug = params.subtype.replace(/-/g, ' ');
  ...
  const where: Prisma.ProductWhereInput = {
    type: { name: typeName },
    subtype: { name: { equals: subtypeSlug, mode: 'insensitive' } }
  };
  ...
  // Fetch data
  const [totalProducts, products, subtypeData] = await Promise.all([
    ...
    prisma.subtype.findFirst({
      where: { name: { equals: subtypeSlug, mode: 'insensitive' } }
    })
  ]);
  ...
  return (
    <div className="container mx-auto px-4 md:px-6 py-8">
      {/* Breadcrumb & Title */}
      <div className="mb-8">
        <h1 className="font-display text-4xl font-bold text-primary-900 mb-2">{subtypeData.name} Sarees</h1>
        <p className="text-primary-700">Explore our curated collection of {subtypeData.name} {typeName.toLowerCase()} sarees.</p>
      </div>
